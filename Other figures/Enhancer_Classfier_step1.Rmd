...
enrichment2D <- function( Var1, Var2, bkg, test, xlab="Variable 1", ylab="Variable 2", const.size=F, logOddsLimit=1, logPLimit=10, color="black" )
{
 ...
 #  merged[6,6] = 1.30103+1e-100
#  merged[5,6] = 10+1e-100
...
#after correcting the overlapping issue with original T and N enhancers. cat should have been used instead merge

#table <- read.csv("resulted enhancers from binomial_test_p_value_combined_added_enhancer_grps_and_othergenes_added_5kbdownstream_new.csv")

#merged 10kb regions and then created new regions from centre
#table <- read.csv("resulted enhancers from binomial test_p_value_combined_promoter5kb_new.edited.csv")

table <- read.csv("resulted enhancers from binomial test_p_value_combined_promoter5kb_new.csv")
...
```{r}
# Recalculate the p-values, but using a two-sided binomial test. earlier it was calculated using the smallest of two binomial tests, one corresponding to "less" and one to "greater" as the alternative hypothesis)
...
#pdf(file="C:/Users/pubud/Dropbox/thesis/graphs/supfig1/supfig1_l_enhancer_cpg_stat_power.pdf", height=5,width=7)
...
hypo.results <- enrichment2D(
  ...
  str_wrap("Overlap with enhancers found in normal samples",width=30),
  str_wrap("Overlap with enhancers found in tumor samples",width=30) )
...
hyper.results <- enrichment2D(
  ...
  str_wrap("Overlap with enhancers found in normal samples",width=30),
  "" )
#pdf(file="C:/Users/pubud/Dropbox/thesis/graphs/fig 2/fig2_a_enhancer_enhancer_overlap_scales.pdf", height=5,width=13)
grid.arrange(
  hypo.results$plot +
    ggtitle(str_wrap("Enrichment of hypo-methylated enhancers",width=30)) ,
  hyper.results$plot +
    ggtitle(str_wrap("Enrichment of hyper-methylated enhancers",width=30)),
  nrow=1)
#dev.off()
```
...
gain_TN <- enrichment2D(
  ...
  str_wrap("Overlap with enhancers found in normal samples",30),
  str_wrap("Overlap with enhancers found in tumor samples",30) )

loss_TN <- enrichment2D(
  table$Overlaps.of.10kb.region.with.query.enhancers.from.normal.samples,
  table$Overlaps.of.10kb.region.with.query.enhancers.from.tumor.samples,
  !is.na(table$Number.of.Overlaps.of.10kb.region.with.reference.lost.enhancers.in.tumor.N.), # the background is the set of enhancers with non-NA overlap number,
  !is.na(table$Number.of.Overlaps.of.10kb.region.with.reference.lost.enhancers.in.tumor.N.) & table$Number.of.Overlaps.of.10kb.region.with.reference.lost.enhancers.in.tumor.N. > 0, # the test is the set of enhancers with at least one overlap with gained enhancers from Yao et al.
  str_wrap("Overlap with enhancers found in normal samples",30),
  "" )
...
loss_methyl <- enrichment2D(
  ...
  str_wrap("Number of hyper-methylated CpGs",30),
  "" )
#pdf(file="C:/Users/pubud/Dropbox/thesis/graphs/fig 2/fig2_e_yaoetal_enhancers_scales.pdf", height=5,width=10)
grid.arrange(
  gain_TN$plot +
    ggtitle(str_wrap("Enrichment of gained enhancers (Yao et al.)",width=30)) +
    guides(fill=F),
  loss_TN$plot +
    ggtitle(str_wrap("Enrichment of lost enhancers  (Yao et al.)",width=30)) +
    guides(fill=F),
  nrow=1)
#dev.off()
#pdf(file="C:/Users/pubud/Dropbox/thesis/graphs/fig 2/fig2_c_enhancer_methylation_scales.pdf", height=6,width=5)
grid.arrange(
  gain_methyl$plot +
    ggtitle(str_wrap("Enrichment of gained enhancers (Yao et al.)",width=30)) +
    guides(fill=F),
...
#pdf(file="C:/Users/pubud/Dropbox/thesis/graphs/supfig2/supfig2_a_enhancer_roc.pdf", height=5,width=8)
# draw the ROC curve
par(mfrow=c(1,2))
...
plot(proc,main=str_wrap("Predicting gained enhancers",20))
#lines( proc$specificities, proc$thresholds, lty=2 )
auc(proc,  partial.auc.focus=c("specificity","sensitivity"))
...
plot(proc,main=str_wrap("Predicting lost enhancers",20))
#lines( proc$specificities, proc$thresholds, lty=2 )
auc(proc,  partial.auc.focus=c("specificity","sensitivity"))
#dev.off()
...
```{r}
 #roc.area( (cv.result[,1]==T), cv.result[,ncol(cv.result)] )
```
...
#pdf(file="C:/Users/pubud/Dropbox/thesis/graphs/fig 3/fig3_a_predicted_enhancer_methylation.pdf", height=5,width=7.4)
grid.arrange(
  gain$plot +
    ggtitle( str_wrap("Predicted gained enhancers", 25) ) +
    guides(fill=F),
  loss$plot +
    ggtitle( str_wrap("Predicted lost enhancers", 20) ) +
    guides(fill=F),
  nochange$plot +
...
nochange <- visualize.predictions.discrete(
  ...
  str_wrap("Overlap with enhancers found in normal samples",30),
  "" )
#pdf(file="C:/Users/pubud/Dropbox/thesis/graphs/fig 3/fig3_a_predicted_enhancer_derrived_tissue.pdf", height=5,width=15.5)
grid.arrange(
  gain$plot +
    ggtitle( str_wrap("Predicted gained enhancers", 25) ) +
    guides(fill=F),
...
gain <- visualize.predictions.discrete(
  ...
  synth.cov$overlap_normal10kb == 1 & synth.cov$overlap_tumor10kb == 1 & synth.predicted[,2] > 0.8,
  str_wrap("Number of hyper-methylated CpGs",30),
...
loss <- visualize.predictions.discrete(
  ...
  synth.cov$hypo10kb,
  synth.cov$overlap_normal10kb == 1 & synth.cov$overlap_tumor10kb == 1,
  synth.cov$overlap_normal10kb == 1 & synth.cov$overlap_tumor10kb == 1 & synth.predicted[,1] > 0.8,
  str_wrap("Number of hyper-methylated CpGs",30),
  "" )
nochange <- visualize.predictions.discrete(
  synth.cov$hyper10kb,
  synth.cov$hypo10kb,
  synth.cov$overlap_normal10kb == 1 & synth.cov$overlap_tumor10kb == 1,
  synth.cov$overlap_normal10kb == 1 & synth.cov$overlap_tumor10kb == 1 & synth.predicted[,1] < 0.8 & synth.predicted[,2] < 0.8,
  str_wrap("Number of hyper-methylated CpGs",30),
  "" )

#pdf(file="C:/Users/pubud/Dropbox/thesis/graphs/supfig3/fig3_a_predicted_stable_enhancer_methylation.pdf", height=5,width=9.8)
grid.arrange(
  gain$plot +
    ggtitle( str_wrap("Predicted gained enhancers", 25) ) +
    guides(fill=F),
  loss$plot +
    ggtitle( str_wrap("Enhancers classified as lost", 25) ) +
    guides(fill=F),
  nochange$plot +
    ggtitle( str_wrap("Predicted stable enhancers", 25) ) +
    guides(fill=F),
...
gain <- visualize.predictions.discrete(
  synth.cov$hyper10kb,
  synth.cov$hypo10kb,
  synth.cov$overlap_normal10kb == 0 & synth.cov$overlap_tumor10kb > 1,
  synth.cov$overlap_normal10kb == 0 & synth.cov$overlap_tumor10kb > 1 & synth.predicted[,2] > 0.8,
  str_wrap("Number of hyper-methylated CpGs",30),
  str_wrap("Number of hyper-methylated CpGs",50) )
loss <- visualize.predictions.discrete(
  synth.cov$hyper10kb,
  synth.cov$hypo10kb,
  synth.cov$overlap_normal10kb == 0 & synth.cov$overlap_tumor10kb > 1,
  synth.cov$overlap_normal10kb == 0 & synth.cov$overlap_tumor10kb > 1 & synth.predicted[,1] > 0.8,
  str_wrap("Number of hyper-methylated CpGs",30),
  "" )
nochange <- visualize.predictions.discrete(
  synth.cov$hyper10kb,
  synth.cov$hypo10kb,
  synth.cov$overlap_normal10kb == 0 & synth.cov$overlap_tumor10kb > 1,
  synth.cov$overlap_normal10kb == 0 & synth.cov$overlap_tumor10kb > 1 & synth.predicted[,1] < 0.8 & synth.predicted[,2] < 0.8,
  str_wrap("Number of hyper-methylated CpGs",30),
  "" )

#pdf(file="C:/Users/pubud/Dropbox/thesis/graphs/supfig3/fig3_e_norm_0_tumor_1_methylation.pdf", height=5,width=9.8)
grid.arrange(
  gain$plot +
    ggtitle( str_wrap("Predicted gained enhancers", 25) ) +
    guides(fill=F),
...
```{r}
write.table( table, "01.gain_loss_enhancers.table.csv", sep=",", quote=T, row.names=F )
save(list=ls(),file="01.gain_loss_enhancers.table.RData")
